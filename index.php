...
if (!defined('ABSPATH')) {
  ...
  define('WP_PLUGIN_DIR_', realpath('.'));
...
$queryck="SELECT details FROM ".$rich_counter_prefix."last WHERE time>'$lasttime' AND ip='$g_addr'";
...
while ($rowck = mysql_fetch_row ($resultck)) {
  ...
  if ($counter_page) {
    ...
    $details=$rowck[0];
  $query="UPDATE ".$rich_counter_prefix."last SET time_last='$nowtime', visits=visits+1, details='".mysql_escape_string($details)."' WHERE time>'$lasttime' AND ip='$g_addr'";
  $saschart->mysqlQuery($query);

  $query="SELECT page FROM ".$rich_counter_prefix."top WHERE month='$nowmonth'";
  $result = $saschart->mysqlQuery($query);
  while ($row = mysql_fetch_row ($result)) {
    $csv_page=$saschart->doCsvTop($row[0], $counter_page);
    $queryup="UPDATE ".$rich_counter_prefix."top SET page='$csv_page', page_rank=page_rank+1 WHERE month='$nowmonth'";
    $saschart->mysqlQuery($queryup);
  }
  mysql_free_result($result);
  if (!$csv_page) {
    $csv_page=mysql_escape_string($counter_page).$csv_cols."1";
    $queryin="INSERT INTO ".$rich_counter_prefix."top SET month='$nowmonth', page='$csv_page', page_rank=1";
...
if ($saschart->S('s_count_step')==1) {
  ...
  $query="INSERT INTO ".$rich_counter_prefix."last SET time='$nowtime', time_last='$nowtime', ip='$g_addr', ref='".mysql_escape_string($counter_ref)."', word='".mysql_escape_string($counter_word)."', browser='$counter_browser', agent='".mysql_escape_string($saschart->SV('HTTP_USER_AGENT'))."', os='$counter_os', res='$counter_res', country='$counter_country', visits=1, details='$details'";
  $saschart->mysqlQuery($query);

  $query="SELECT ip FROM ".$rich_counter_prefix."last";
  $result = $saschart->mysqlQuery($query);
  $num_entries=mysql_num_rows($result);
  if ($num_entries>$last_entries) {
    $del_entries=$num_entries-$last_entries;
    $querydel="DELETE FROM ".$rich_counter_prefix."last ORDER BY time LIMIT $del_entries";
    $saschart->mysqlQuery($querydel);
  }
  mysql_free_result($result);

  $queryck="SELECT ip FROM ".$rich_counter_prefix."hold WHERE ip='$g_addr'";
  $resultck = $saschart->mysqlQuery($queryck);
  if (!mysql_num_rows($resultck)) {

    $queryin="INSERT INTO ".$rich_counter_prefix."hold SET time='$nowtime', ip='$g_addr'";
    $saschart->mysqlQuery($queryin);

    $querydel="DELETE FROM ".$rich_counter_prefix."hold WHERE time<='$lasttime'";
    $saschart->mysqlQuery($querydel);

    $query="SELECT hours, days, months, years FROM ".$rich_counter_prefix."time WHERE hours<>'' LIMIT 1";
    $result = $saschart->mysqlQuery($query);
    while ($row = mysql_fetch_row ($result)) {
      $queryup="UPDATE ".$rich_counter_prefix."time SET
      ...
      if ($csv_clean=="days" or $csv_clean=="months") {
        ...
        $queryin="INSERT INTO ".$rich_counter_prefix."time SET days='$row[1]', months='$lastmonth'";
        $saschart->mysqlQuery($queryin);
      }
      if ($csv_clean=="months") {
        $lastyear=substr($row[2],0,2);
        $queryin="INSERT INTO ".$rich_counter_prefix."time SET months='$row[2]', years='$lastyear'";
        $saschart->mysqlQuery($queryin);
      }
      $counter_time=1;
    }
    mysql_free_result($result);
    if (!$counter_time) {
      $queryin="INSERT INTO ".$rich_counter_prefix."time SET
    ...
    $query="SELECT page, ref, word, robot, browser, os, res, country FROM ".$rich_counter_prefix."top WHERE month='$nowmonth' LIMIT 1";
    $result = $saschart->mysqlQuery($query);
    while ($row = mysql_fetch_row ($result)) {
      $queryup="UPDATE ".$rich_counter_prefix."top SET ";
      if ($counter_page)
    ...
    mysql_free_result($result);
    if (!$counter_top) {
      $queryin="INSERT INTO ".$rich_counter_prefix."top SET month='$nowmonth',";
      if ($counter_page)
